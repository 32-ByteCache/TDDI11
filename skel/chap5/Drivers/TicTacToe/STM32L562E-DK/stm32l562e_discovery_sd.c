/**
  ...
  * @brief   This file includes the uSD card driver.
  ...
  (#) Initialization steps:
       (++) Initialize the micro SD card using the TicTacToe_SD_Init() function. This
            function includes the MSP layer hardware resources initialization (TicTacToe_SD_MspInit())
            and the SDMMC1 interface configuration to interface with the external micro SD. It
            also includes the micro SD initialization sequence.
       (++) To check the SD card presence you can use the function TicTacToe_SD_IsDetected() which
            returns the detection status.
       (++) If SD presence detection interrupt mode is desired, you must configure the
            SD detection interrupt mode by calling the functions TicTacToe_SD_ITConfig().
       ...
       (++) The function TicTacToe_SD_GetCardInfo() is used to get the micro SD card information
  ...
  (#) Micro SD card operations
       (++) The micro SD card can be accessed with read/write block(s) operations once
            ...
            mode by calling the functions TicTacToe_SD_ReadBlocks()/TicTacToe_SD_WriteBlocks(),
            or by DMA transfer using the functions TicTacToe_SD_ReadBlocks_DMA()/TicTacToe_SD_WriteBlocks_DMA().
       (++) The DMA transfer complete is used with interrupt mode. Once the SD transfer
            ...
            TicTacToe_SD_WriteCpltCallback()/TicTacToe_SD_ReadCpltCallback() user callback functions implemented
            by the user at application level.
       (++) The SD erase block(s) is performed using the function TicTacToe_SD_Erase() with specifying
            the number of blocks to erase.
       (++) The SD runtime status is returned when calling the function TicTacToe_SD_GetStatus().
   [..]
  @endverbatim
  ******************************************************************************
  * @attention
...
/** @defgroup STM32L562E-DK_SD_Private_TypesDefinitions STM32L562E-DK SD Private Types Definitions
  * @{
  */
typedef void (* TicTacToe_SD_EXTI_Callback)(void);
/**
  * @}
  */
...
int32_t TicTacToe_SD_Init(uint32_t Instance)
{
  ...
  else
  {
    ...
    if((uint32_t)TicTacToe_SD_IsDetected(Instance) != SD_PRESENT)
    {
      ret = TicTacToe_ERROR_UNKNOWN_COMPONENT;
    }
    else
    {
      ...
      {
        if(TicTacToe_SD_RegisterDefaultMspCallbacks(Instance) != TicTacToe_ERROR_NONE)
        {
...
#if (USE_HAL_SD_REGISTER_CALLBACKS == 1)
/**
  * @brief TicTacToe SD default Msp Callback registering
  * @param Instance     SD Instance
  * @retval TicTacToe status
  */
...
/**
  * @brief TicTacToe SD Msp Callback registering
  * @param Instance     SD Instance
...
int32_t TicTacToe_SD_DetectITConfig(uint32_t Instance)
{
  ...
  TicTacToe_SD_EXTI_Callback SdDetectCallback[SD_INSTANCES_NBR] = {SDMMC1_DETECT_Callback};
  ...
  else
  {
    ...
    if (HAL_EXTI_GetHandle(&hsd_exti[Instance], LineDetect[Instance]) == HAL_OK)
    {
      if (HAL_EXTI_RegisterCallback(&hsd_exti[Instance], HAL_EXTI_COMMON_CB_ID, SdDetectCallback[Instance]) == HAL_OK)
      {
        ...
        HAL_NVIC_SetPriority(SdDetectIrq[Instance], TicTacToe_SD_IT_PRIORITY, 0);
...
/**
  * @brief  TicTacToe SD Callback.
  * @param  Instance SD Instance
  * @param  Status   Pin status
  * @retval None.
  */
__weak void TicTacToe_SD_DetectCallback(uint32_t Instance, uint32_t Status)
{
...
/**
  * @brief  TicTacToe SD detect interrupt handler.
  * @param  Instance SD Instance.
  * @retval None.
  */
...
/**
  * @brief  TicTacToe SD interrupt handler.
  * @param  Instance SD Instance.
  * @retval None.
  */
...
/**
  * @brief TicTacToe SD Abort callback
  * @param  Instance     SD Instance
  * @retval None
  */
__weak void TicTacToe_SD_AbortCallback(uint32_t Instance)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(Instance);
}
...
/**
  * @brief TicTacToe Tx Transfer completed callback
  * @param  Instance     SD Instance
  * @retval None
  */
__weak void TicTacToe_SD_WriteCpltCallback(uint32_t Instance)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(Instance);
}
...
/**
  * @brief TicTacToe Rx Transfer completed callback
  * @param  Instance     SD Instance
  * @retval None
  */
__weak void TicTacToe_SD_ReadCpltCallback(uint32_t Instance)
{
  /* Prevent unused argument(s) compilation warning */
  UNUSED(Instance);
}
...
static void SDMMC1_DETECT_Callback(void)
{
  ...
  {
    TicTacToe_SD_DetectCallback(0, SD_NOT_PRESENT);
  }
  else
  {
    TicTacToe_SD_DetectCallback(0, SD_PRESENT);
  }
}
...
static void SD_MspInit(SD_HandleTypeDef *hsd)
{
  ...
  if (ret == HAL_OK)
  {
    ...
    if (ret == HAL_OK)
    {
      ...
      HAL_NVIC_SetPriority(SDMMCx_IRQn, TicTacToe_SD_IT_PRIORITY, 0);
...
static void SD_AbortCallback(SD_HandleTypeDef *hsd)
{
  ...
  do
  {
    ...
    {
      TicTacToe_SD_AbortCallback(instance);
    }
    instance ++;
...
static void SD_TxCpltCallback(SD_HandleTypeDef *hsd)
{
  ...
  do
  {
    if (hsd == &hsd_sdmmc[instance])
    {
      TicTacToe_SD_WriteCpltCallback(instance);
    }
    instance ++;
  }while (instance < SD_INSTANCES_NBR);
}
...
static void SD_RxCpltCallback(SD_HandleTypeDef *hsd)
{
  ...
  do
  {
    if (hsd == &hsd_sdmmc[instance])
    {
      TicTacToe_SD_ReadCpltCallback(instance);
    }
    instance ++;
  }while (instance < SD_INSTANCES_NBR);
}
