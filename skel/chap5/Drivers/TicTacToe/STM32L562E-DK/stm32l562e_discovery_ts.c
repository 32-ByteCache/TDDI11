  ...
  *          Screen on STM32L562E-DISCOVERY board.
  ...
  ---------------------
    + Initialization steps:
       o Initialize the TS using the TicTacToe_TS_Init() function. You can select
    ...
    + Touch screen use
       o Call TicTacToe_TS_EnableIT() (TicTacToe_TS_DisableIT()) to enable (disable) touch
         screen interrupt. TicTacToe_TS_Callback() is called when TS interrupt occurs.
       o Call TicTacToe_TS_GetState() to get the current touch status (detection and
         coordinates).
       o Call TicTacToe_TS_Set_Orientation() to change the current orientation.
         Call TicTacToe_TS_Get_Orientation() to get the current orientation.
       o Call TicTacToe_TS_GetCapabilities() to get the FT6X06 capabilities.
       o Call TicTacToe_TS_Get_MultiTouchState() to get the current touches and events.
       o FT6X06 doesn't support gesture feature.
         TicTacToe_TS_GestureConfig() and TicTacToe_TS_GetGestureId() functions will return 
         TicTacToe_ERROR_FEATURE_NOT_SUPPORTED.
 
    + De-initialization steps:
       o De-initialize the touch screen using the TicTacToe_TS_DeInit() function.
...
/**
  ...
  * @retval None.
  */
__weak void TicTacToe_TS_Callback(uint32_t Instance)
{  
